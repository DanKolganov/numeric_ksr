...
struct config {   
    config(double x_min, double x_max, double x_0, double u_0, double du_0, double step, uint N_max, bool LEC, double eps, double A, double B, double C) : x_min(x_min), x_max(x_max), x_0(x_0), u_0(u_0), du_0(du_0), step(step), N_max(N_max), LEC(LEC), eps(eps), A(A), B(B), C(C) {}
...
static double task21_rhs(double x, double u, double du) {
...
std::tuple<double, double> find_min_u(const resultTable& tbl);
